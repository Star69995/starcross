...
- Every new or changed UI must work on small phone widths (~350px), regular phone widths (~360–430px), and iPad widths (~768–1024px), not just desktop — use Bootstrap's grid/breakpoints (`col-*`, `d-*-none`, container padding) rather than fixed pixel widths.
...
- Text alignment: no bare left/right default. Under `dir="rtl"` with no `text-align` set, block text resolves to plain right-alignment (`App.css`'s `body { text-align: center; }` already covers this baseline) — every text block should read as either **centered** (the default: headings, labels, list items, table cells, short copy) or **justified** (flowing multi-line paragraphs/descriptions, via the `p { text-align: justify; }` rule). Editable fields (`.form-control`/`.form-select`/`textarea`) are the deliberate exception and stay right-aligned, since centering live-typed text is disorienting. If a component needs its own right/left alignment for a real layout reason (e.g. keeping label text hugging a leading icon, like `.sound-option-btn`), that's fine but comment why — it's an exception to this rule, not the default.
